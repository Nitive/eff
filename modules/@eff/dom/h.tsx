...
export function createElement<Props, Sources extends { DOM: any }, Sinks extends { DOM: Stream<VNode> }>(
  ...
  return (sources: Sources) => {
    ...
    if (!DOM) {
      return { ...sinks, ...(component(props as any, sources)(sources) as any) }
    }

    const sinks$ = DOM
      .map(ch => {
        const result = component(Object.assign(props as any, { children: ch }), sources)
        if (Array.isArray(result)) {
          return xs.of(getChildren(result, sources)) as Stream<Sinks>
        }
        return xs.of(result(sources))
      })
      .flatten()
